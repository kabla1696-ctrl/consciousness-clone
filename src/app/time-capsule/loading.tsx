    ...
    <main className="min-h-screen pb-24 md:pb-8" style={{ background: '#050510' }}>
      ...
      <div className="px-4 py-6 space-y-6 relative z-10">
        ...
        <div className="bg-white/[0.03] rounded-2xl border border-white/[0.06] p-6 text-center space-y-4">
          <div className="h-5 w-40 mx-auto rounded bg-white/5 shimmer" />
          <div className="flex justify-center gap-3">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="w-16 h-20 rounded-xl bg-white/5 shimmer border border-white/[0.06]" />
            ))}
          </div>
          <div className="h-3.5 w-48 mx-auto rounded bg-white/3 shimmer" />
        </div>
          ...
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-white/[0.03] rounded-2xl border border-white/[0.06] p-5">
              ...
              <div className="flex items-center justify-between">
                <div className="h-3 w-28 rounded bg-white/3 shimmer" />
                <div className="h-7 w-20 rounded-lg bg-white/5 shimmer" />
              </div>
            </div>
          ))}
        </div>
